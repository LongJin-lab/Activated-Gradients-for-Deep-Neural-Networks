...
from GAF import SGD_atan, SGD_atanMom, Adam_atan
from Adam import Adam_atan
from RMSprop import RMSprop
from RandLR import RandLR
from models import *
...
parser.add_argument("--RandType", default='uniform', type=str)
parser.add_argument("--LrSche", default='RandLR', type=str)
args = parser.parse_args()
args.save_path = 'runs/cifar10/'+args.arch+str(args.LrSche)+'RandType'+str(args.RandType)+\
                                  '/Alpha'+str(args.alpha)+\
                                  'Beta'+str(args.beta)+str(args.opt)+\
...
#        optimizer = torch.optim.RMSprop(net.parameters(), lr=args.lr, alpha=0.99, eps=1e-08, weight_decay=5e-4, momentum=0.9)
    if args.LrSche == 'RandLR':
        print('args.RandType', args.RandType)
        train_scheduler = RandLR(optimizer, args.RandType)
    elif args.LrSche == 'CosineAnn':
